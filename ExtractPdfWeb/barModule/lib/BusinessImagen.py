...
class BusinessImagen():
    ...
    def contarImagenesGeneradas(self):
        ...
        """
        number_of_images = len(glob.glob( IMAGENES_PATH + configuracion['rutas_de_todas_imagenes']))
    ...
    def cargaImagen (self, count):
        ...
        if os.path.exists(IMAGENES_PATH + 'image_name.jpg'):
            image = cv2.imread(IMAGENES_PATH + 'image_name.jpg')
        else:
            image = cv2.imread(IMAGENES_PATH + 'image_name-' + count.__str__() + '.jpg')
...
#        os.remove(filename)
    ...
    def borrarImagenesCreadas(self):
        ...
        for imagen in glob.glob(IMAGENES_PATH +configuracion['rutas_de_todas_imagenes']):
            os.remove(imagen)
